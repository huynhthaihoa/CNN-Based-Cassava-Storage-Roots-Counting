...
def transition_block(x, reduction, name):
    ...
    x = SeparableConv2D(int(K.int_shape(x)[bn_axis] * reduction), 1,
                      use_bias=False,
                      name=name + '_conv')(x)
...
def conv_block(x, growth_rate, name):
    ...
    x1 = SeparableConv2D(4 * growth_rate, 1,
                       use_bias=False,
...
def DenseNet(blocks, input_shape=(para.img_rows, para.img_cols, para.channels), classes=para.type_classes):
